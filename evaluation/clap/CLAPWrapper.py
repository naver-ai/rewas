...
class CLAPWrapper():
    ...
    def load_audio_into_tensor(self, audio_path, audio_duration, resample=False, cutoff=5):
        r"""Loads audio file and returns raw audio."""
        # Randomly sample a segment of audio_duration from the clip or pad to match duration
        audio_time_series, sample_rate = torchaudio.load(audio_path)
        audio_time_series = audio_time_series[:sample_rate*cutoff]
        return self.resample_and_duration((audio_time_series, sample_rate),audio_duration,resample)

    def preprocess_audio(self, audio_files, resample, cutoff=5):
    ...
    def get_audio_embeddings(self, audio_files, resample, cutoff=5):
        r"""Load list of audio files and return a audio embeddings"""
        preprocessed_audio = self.preprocess_audio(audio_files, resample, cutoff)
