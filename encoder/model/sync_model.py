...
from encoder.model.modules.transformer import Block, Config
...
class Synchformer(torch.nn.Module):
    ...
    def forward(self, vis: torch.Tensor, aud: torch.Tensor, targets: torch.Tensor = None, for_loop=False,
        ...
        '''
        #  'segment_size_vframes': 16, 'n_segments': 14,
        # vis = vis.repeat(1,2,1,1,1,1)
        print(f"input {vis.shape}") # input torch.Size([1, 14, 16, 3, 224, 224])
        # vis = vis.repeat(1,1,2,1,1,1) # 이건 안되고 14 여기만 높여야 한다.
        
        vis = self.extract_vfeats(vis, for_loop, vis_mask=vis_mask)
        print(f"vis {vis.shape}")# 1, 14, 8, 768
        aud = self.extract_afeats(aud, for_loop, aud_mask=aud_mask)

        vis = self.vproj(vis)
        print(f"proj vis {vis.shape}") # 1, 14, 8, 768 = 1, S, T, D
