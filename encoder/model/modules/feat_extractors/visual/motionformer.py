...
import einops
import sys
sys.path.append("./encoder/model/modules/feat_extractors/visual")
from motionformer_src.video_model_builder import VisionTransformer
from encoder.encoder_utils import check_if_file_exists_else_download
...
class MotionFormer(VisionTransformer):
    ...
    def forward_segments(self, x, orig_shape: tuple, cont_mask: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.extract_features: # true
            ...
            x = self.norm(x)
            # x torch.Size([14, 1568, 768])
            x = self.pre_logits(x)
            # print(f"pre_logits {x.shape}") # pre_logits torch.Size([14, 1568, 768])
            if self.factorize_space_time: # true
                x = self.restore_spatio_temp_dims(x, orig_shape)  # (B*S, D, t, h, w) <- (B*S, t*h*w, D)
                # print(f"recon {x.shape}") # recon torch.Size([14, 768, 8, 14, 14])
                if cont_mask is not None:
                ...
                # print(f"temporal {x.shape}") # spatial torch.Size([14, 8, 768])
        else:
