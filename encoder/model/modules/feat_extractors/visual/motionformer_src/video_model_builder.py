...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, cont_mask=None):
        # print(f"input2 {x.shape}") # input2 torch.Size([1, 14, 3, 16, 224, 224])
        if self.video_input:
            x = x[0]
        ...
        if self.cfg.VIT.PATCH_SIZE_TEMP > 1:
            ...
            # print(x.shape) #torch.Size([14, 3, 16, 224, 224])
            x = self.patch_embed_3d(x)
            # print(x.shape) # torch.Size([14, 1568, 768])
            # input()
        ...
        # print(f"tokenize {x.shape}") # tokenize torch.Size([14, 1568, 768])
        # Append CLS token
