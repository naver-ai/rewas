...
import audioldm.utilities.audio as Audio
from utils import load_json
from encoder.encoder_utils import get_transforms, which_ffmpeg, patch_config
from audioldm.utilities.data.utils import re_encode_video, get_video_and_audio
...
class AudioDataset(Dataset):
    def __init__(
        ...
        self.dropout = config["data"]["dropout"]
        self.build_dsp()
        self.label_num = len(self.index_dict)
        self.train_mode = config["data"]["train_mode"]

        exp_name = config["synchformer_exp_name"]
    ...
    def __getitem__(self, index):
        ...
        data = {
            ...
            "log_mel_spec": "" if (log_mel_spec is None) else log_mel_spec.float(), # this will be used for training
        ...
        if self.train_mode == "energy":
            control = torch.mean(data["log_mel_spec"], axis=1)
            control = control.unsqueeze(0).unsqueeze(0)
            control = F.interpolate(control, size = 112)
            control = savgol_filter(control.cpu(),9,2) 
            control = torch.tensor(control)
            control = F.interpolate(control, size = 512)
            data["hint"] = torch.tensor(control)

        if self.train_mode == "video_energy":
            control = torch.mean(data["log_mel_spec"], axis=1)
            control = control.unsqueeze(0).unsqueeze(0)
            control = F.interpolate(control, size = 112)
            control = savgol_filter(control.cpu(),9,2) 
            data["image"] = torch.tensor(control)

            vfps = 25
            afps = 16000
            in_size = 256
            duration = 5
            video = self.data[index]["wav"]
            waveform, sr = torchaudio.load(video)
            video = video.replace("audiodata", "videodata")
            video = video.replace("wav", "mp4")

            video_path = self.data_root +"/" + video.split("/")[-1]
            video_path = video_path.replace('.was', '_25fps_256side_16000hz.mp4')

            if os.path.exists(video_path):
                rgb, audio, meta = get_video_and_audio(
                    video_path, get_meta=True, duration=duration, start_sec=0, end_sec=None, random_start = random_start/waveform.shape[1])
            else:
                video_path = re_encode_video(self.data_root, video, vfps, afps, in_size)
                rgb, audio, meta = get_video_and_audio(
                    video_path, get_meta=True, duration=duration, start_sec=0, end_sec=None, random_start = random_start/waveform.shape[1])
            ...
            item_temp = get_transforms(self.cfg, ['test'])['test'](item_temp) 
            
            video = item_temp['video'].unsqueeze(0)
            data["hint"] = video_encoder
    ...
    # def get_video_and_audio(self, path, get_meta=False, duration=5, start_sec=0, end_sec=None, random_start = None):
    ...
    #     else:
    #         random_start *= rgb.shape[0]
    #         random_start = int(random_start)
    #         audio = audio[random_start*meta['audio_fps']:(random_start*meta['audio_fps']+alen)]
    #         rgb = rgb[int(random_start):int(random_start)+vlen, :, :, :]
